...
rrms_shannon<- estimate_richness(rrms_alone, measures = c("Shannon"))
...
wilcox.test(PD ~ sex, data= phylo_dist)
...
#### Beta diversity #####
#### Produce an ordination figure for weighted unifrac #####
ord_rrms <- distance(rrms_alone, method="wunifrac")
...
adonis2(ord_rrms ~ sex,
        data = data.frame(sample_data(rrms_alone)))
